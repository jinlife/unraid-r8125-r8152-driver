...
static int rtl_fc_pause_pkt_en(struct r8152 *tp)
{
	int ret, ratio, temp;
	u16 speed, num_pause_pkts;
	u32 ocp_data;
	...
	case RTL_VER_14:
		...
		ocp_data = tp->version == RTL_VER_09 ? FLOW_CTRL_PATCH_OPT :
						       FLOW_CTRL_PATCH_2;
...
static int r8153_init(struct r8152 *tp)
{
	...
	ocp_data |= (tp->udev->speed < USB_SPEED_SUPER) ? LPM_TIMER_1MS :
							  LPM_TIMER_500US;
	ret = ocp_byte_set_bits(tp, MCU_TYPE_USB, USB_LPM_CTRL, (u8)ocp_data);
	if (ret < 0)
		goto out;
