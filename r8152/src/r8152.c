...
#define DRIVER_VERSION "v2.22.1"DRIVER_SUFFIX" (2026/06/03)"
...
#define LPM_TIMER_1MS		0x08	/* 1 ms */
...
#define RTL8152_TXFG_HEADSZ	256
...
#define RTL_MAX_SG_NUM		(MAX_SKB_FRAGS + 1)
...
#define VENDOR_ID_ASUSTEK               0x0b05
			 ...
			 struct sockaddr_storage *ss)
{
	...
	memcpy(ss->__data, buf, sizeof(buf));
					 ...
					 struct sockaddr_storage *ss)
{
	...
	if (tp->dell_macpassthru)
		...
		ret = rtl_mapt_read(tp, "\\MACA", ACPI_TYPE_STRING, 0x16, ss);

	return ret;
}

static int rtl_hw_ether_addr(struct r8152 *tp, struct sockaddr_storage *ss)
{
	u32 ocp_data = 0;
	int ret;
	...
	if (tp->version == RTL_VER_05) {
		...
		if (ocp_data & BIT(0))
			return usb_ocp_read(tp, 0xcf08, 8, ss->__data);

		ocp_data |= BIT(0);
	}

	ret = pla_ocp_read(tp, PLA_BACKUP, 8, ss->__data);
	if (ret < 0)
		goto out;
...
static void rtl_drop_tx_skb(struct r8152 *tp, struct sk_buff *skb)
{
	struct net_device_stats *stats;
...
static int rtl_cal_sg_number(struct sk_buff *skb, unsigned int recursion_level)
{
	...
	number += skb_shinfo(skb)->nr_frags;

	skb_walk_frags(skb, frag_iter) {
		int ret;

		ret = rtl_cal_sg_number(frag_iter, recursion_level + 1);
		if (ret < 0)
			return ret;
...
static int rtl_cal_sg_size(struct r8152 *tp, struct sk_buff *skb,
			   int num, int *len, unsigned int recursion_level)
{
	struct sk_buff *frag_iter;
	int i;
	...
	skb_walk_frags(skb, frag_iter) {
		num = rtl_cal_sg_size(tp, frag_iter, num, len,
				      recursion_level + 1);
		if (num <= 0)
			break;
	}
...
static int rtl_skb_check(struct r8152 *tp, struct sk_buff **pskb, int headroom)
{
	int sg_num, size, eat, nhead, ret, avg_sg;
	struct sk_buff *skb = *pskb;

	ret = rtl_cal_sg_number(skb, 0);
	if (ret < 0)
		goto out;

	sg_num = ret;
	avg_sg = min_t(int, (skb->len + tp->tx_copybreak - 1) / tp->tx_copybreak,
		       RTL_MAX_SG_NUM);

	nhead =  headroom - skb_headroom(skb);

	if (skb_header_cloned(skb) || nhead > 0) {
		if (avg_sg == 1) {
			...
			*pskb = skb = tx_skb;
			...
			ret = 0;
			goto out;
		} else if (avg_sg >= sg_num) {
			ret = pskb_expand_head(skb, nhead > 0 ? nhead : 0, 128,
					       GFP_ATOMIC);
			goto out;
		}
	} else if (avg_sg >= sg_num) {
	...
	ret = rtl_cal_sg_size(tp, skb, sg_num - avg_sg, &size, 0);
	if (ret < 0)
		goto out;

	WARN_ON(ret);

	netif_dbg(tp, tx_queued, tp->netdev,
		  "rtl_skb_check: len=%d, sg_num=%d -> %d, pull size=%d\n",
		  skb->len, sg_num, avg_sg, size);

	/* Although __pskb_pull_tail() may call pskb_expand_head(), the field
	 * of nhead is always filled by 0. I call pskb_expand_head() before
	 * __pskb_pull_tail() to avoid calling it again later if I need to
	 * expand the room of head.
	 */
	eat = (skb->tail + size) - skb->end;
	if (skb_header_cloned(skb) || nhead > 0 || eat > 0) {
		ret = pskb_expand_head(skb, nhead > 0 ? nhead : 0,
				       eat > 0 ? eat + 128 : 0, GFP_ATOMIC);
		if (ret < 0)
			goto out;
	}

	ret = __pskb_pull_tail(skb, size) ? 0 : -ENOMEM;
...
static int rx_bottom(struct r8152 *tp, int budget)
{
	...
	if (!skb_queue_empty_lockless(&tp->rx_queue)) {
...
static void rtl_drop_queued_tx(struct r8152 *tp)
{
	...
	if (skb_queue_empty_lockless(tx_queue))
		return;
	...
	while (!skb_queue_empty(&skb_head))
		rtl_drop_tx_skb(tp, __skb_dequeue(&skb_head));
}
...
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(5,6,0) */
{
	...
	netif_warn(tp, tx_err, netdev,
		   "Tx timeout: back/submit = %llu/%llu, tx queue = %d\n",
		   tp->tx_back, tp->tx_submit, skb_queue_len(&tp->tx_queue));
...
static bool rtl_gso_check(struct net_device *dev, struct sk_buff *skb)
{
	struct r8152 *tp = netdev_priv(dev);
				      ...
				      struct net_device *netdev)
{
	...
	if (unlikely(!test_bit(WORK_ENABLE, &tp->flags) ||
...
static int rtl_enable(struct r8152 *tp)
{
	...
	WARN_ON(skb_queue_len_lockless(&tp->tx_queue));
...
static int r8156_ups_flags(struct r8152 *tp)
{
	...
	if (tp->ups_info.eee) {
		...
		case RTL_VER_16:
			ups_flags |=
				UPS_FLAGS_EN_100M_EEE | UPS_FLAGS_EN_1000M_EEE |
				UPS_FLAGS_EN_2500M_EEE | UPS_FLAGS_EN_5000M_EEE;
			break;
		case RTL_VER_13:
		case RTL_VER_15:
		default:
...
static int r8153b_ups_en(struct r8152 *tp, bool enable)
{
	...
	if (enable) {
		...
		if (ocp_data & PCUT_STATUS) {
			...
			ret = ocp_write_word(tp, MCU_TYPE_USB, USB_FW_PLA_VER, 0);
			if (ret < 0)
				goto out;
...
static int r8157_ups_en(struct r8152 *tp, bool enable)
{
	int ret;

	netif_dbg(tp, drv, tp->netdev, "%s:%sable UPS\n", __func__,
		  enable ? " en" : " dis");
	if (enable) {
		ret = r8156_ups_flags(tp);
		if (ret < 0)
		...
		netif_dbg(tp, drv, tp->netdev, "%s: %sresume\n", __func__,
			  ocp_data & PCUT_STATUS ? "UPS mode " : "");

		if (ocp_data & PCUT_STATUS) {
			ret = ocp_write_byte(tp, MCU_TYPE_USB, USB_FW_USB_VER, 0);
			if (ret < 0)
				goto out;
...
static bool rtl_check_fw_ver_ok(struct r8152 *tp, u16 index, u8 new_ver)
{
	...
	dev_dbg(&tp->intf->dev, "patch code version at %#0x = %#0x\n", index,
		ocp_data);
...
static int rtl_fc_pause_pkt_en(struct r8152 *tp, u16 speed)
{
	int ret, ratio, temp;
	u16 num_pause_pkts;
	u32 ocp_data;
	...
	case RTL_VER_14:
		...
		ocp_data = tp->version == RTL_VER_14 ? FLOW_CTRL_PATCH_2 :
						       FLOW_CTRL_PATCH_OPT;
	...
	case RTL_VER_17_QFN100:
		...
		if (!(speed & LINK_STATUS)) {
			...
			dev_dbg(&tp->intf->dev, "Unknown link speed\n");
			goto no_link;
		}
		...
		/* Round up if it is more than halfway to the next power of 2.
		 * Floating-point is avoided by rewriting ratio > 1.5 * 2^num_pause_pkts
		 * as 2 * ratio > 3 * 2^num_pause_pkts */
		if (2 * ratio > 3 * (1 << num_pause_pkts))
			num_pause_pkts--;

no_link:
		ret = ocp_word_w0w1(tp, MCU_TYPE_PLA, 0xe61e, 0xf0 | BIT(3),
				    num_pause_pkts << 4);
		if (ret < 0)
			break;
...
static void r8153_firmware(struct r8152 *tp)
{
	...
	if (tp->version == RTL_VER_03) {
		...
		ocp_write_word(tp, MCU_TYPE_USB, 0xfc34, 0x068e);
		ocp_write_word(tp, MCU_TYPE_USB, 0xfc36, 0x2b56);
		ret = ocp_read_byte(tp, MCU_TYPE_USB, USB_CSTMR, &ocp_data);
		if (ocp_data & FORCE_SUPER)
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_EN, 0x00fd);
		else
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_EN, 0x00fc);
		...
		ocp_write_word(tp, MCU_TYPE_PLA, 0xfc2e, 0x087a);
		ocp_write_word(tp, MCU_TYPE_PLA, 0xfc30, 0x0000);
...
static int rtl8157_enable(struct r8152 *tp)
{
	...
	speed = rtl8152_get_speed(tp);
	ret = rtl_fc_pause_pkt_en(tp, speed);
	if (ret < 0)
		goto out;
...
static int rtk_disable_diag(struct r8152 *tp)
{
	...
	if (tp->support_2500full || tp->support_5000full || tp->support_10gfull)
		if (r8156_mdio_force_mode(tp) < 0)
			netif_err(tp, drv, tp->netdev, "failed to set force mode\n");
...
static int r8153_init(struct r8152 *tp)
{
	...
	ocp_data |= (tp->udev->speed < USB_SPEED_SUPER) ? LPM_TIMER_1MS :
							  LPM_TIMER_500US;
	ret = ocp_write_byte(tp, MCU_TYPE_USB, USB_LPM_CTRL, (u8)ocp_data);
	if (ret < 0)
		goto out;

	ret = ocp_write_byte(tp, MCU_TYPE_USB, 0xcbd2, ocp_data);
	if (ret < 0)
		goto out;

	if (tp->version == RTL_VER_04 || tp->version == RTL_VER_05) {
		ret = ocp_word_w0w1(tp, MCU_TYPE_USB, USB_AFE_CTRL2,
				    SEN_VAL_MASK, SEN_VAL_NORMAL | SEL_RXIDLE);
		if (ret < 0)
			goto out;
	}
...
static void r8159_patch_code(struct r8152 *tp)
{
	if (tp->version == RTL_VER_17_QFN68) {
		...
		/* PLA/USB ISRAM Checksum Disable */
		ocp_byte_set_bits(tp, MCU_TYPE_USB, 0xb8f2, BIT(0));
		...
		if (rtl_check_fw_ver_ok(tp, USB_FW_USB_VER, new_ver)) {
			...
			generic_ocp_write(tp, 0xe600, 0xff,
					  sizeof(usb_patch_17_QFN68),
					  usb_patch_17_QFN68, MCU_TYPE_USB);

			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_BA, 0xfc00);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_0, 0x89d2);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_1, 0x9344);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_2, 0x0406);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_3, 0x6d7a);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_4, 0x7e54);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_5, 0x05b2);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_6, 0x168c);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_7, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_8, 0xad88);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_9, 0x0978);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_10, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_11, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_12, 0x1c44);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_13, 0x875a);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_14, 0x89b8);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_15, 0x1ab8);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP2_EN, 0xf37f);
			ocp_write_byte(tp, MCU_TYPE_USB, USB_FW_USB_VER,
				       new_ver);
		}
		...
		if (rtl_check_fw_ver_ok(tp, USB_FW_PLA_VER, new_ver)) {
			...
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_BA, 0x8000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_0, 0x5294);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_1, 0x0000);
			...
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_14, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_15, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP2_EN, 0x0001);
			ocp_write_byte(tp, MCU_TYPE_USB, USB_FW_PLA_VER,
				       new_ver);
		}
	} else if (tp->version == RTL_VER_17_QFN100) {
		static u8 usb_patch_17_QFN100[] = {
			0x04, 0xe0, 0x09, 0xe0,
			0x0d, 0xe0, 0x11, 0xe0,
			0x05, 0xc7, 0xe0, 0x76,
			0x02, 0xc7, 0x00, 0xbf,
		...
		if (rtl_check_fw_ver_ok(tp, USB_FW_USB_VER, new_ver)) {
			...
			generic_ocp_write(tp, 0xe600, 0xff,
					  sizeof(usb_patch_17_QFN100),
					  usb_patch_17_QFN100, MCU_TYPE_USB);
			...
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_8, 0xad88);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_9, 0x0978);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_10, 0xfabe);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_11, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_12, 0xf9aa);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_13, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_14, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP_15, 0x0000);
			ocp_write_word(tp, MCU_TYPE_USB, USB_BP2_EN, 0x177f);
			ocp_write_byte(tp, MCU_TYPE_USB, USB_FW_USB_VER,
				       new_ver);
		}
		...
		if (rtl_check_fw_ver_ok(tp, USB_FW_PLA_VER, new_ver)) {
			...
			generic_ocp_write(tp, 0xf800, 0xff, sizeof(pla_patch_17),
					  pla_patch_17, MCU_TYPE_PLA);

			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_BA, 0x8000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_0, 0x530a);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_1, 0x5294);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_2, 0x51c2);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_3, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_4, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_5, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_6, 0x0000);
			...
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_14, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP_15, 0x0000);
			ocp_write_word(tp, MCU_TYPE_PLA, PLA_BP2_EN, 0x0006);
			ocp_write_byte(tp, MCU_TYPE_USB, USB_FW_PLA_VER,
				       new_ver);
		}
	}
...
static int rtl_ram_code_speed_up(struct r8152 *tp, bool wait)
{
	...
	} else if (tp->version == RTL_VER_17_QFN68 ||
		   tp->version == RTL_VER_17_QFN100) {
		static u8 ram17[] = {
			0x6c, 0xe8, 0x00, 0xa0,
			0x36, 0xb4, 0x23, 0x80,
			0x38, 0xb4, 0x00, 0x61,
	...
	dev_dbg(&tp->intf->dev, "ram code version = %#0x\n", fw_version);
...
static int r8156_init(struct r8152 *tp)
{
	...
	ret = ocp_byte_clr_bits(tp, MCU_TYPE_PLA, 0xe0c8, BIT(0));
	if (ret < 0)
		goto out;
...
static int r8156b_u2phy_backup(struct r8152 *tp)
{
	static u8 u2phy_backup[] = {
		0x00, 0x00, 0x1b, 0x18,
		0x99, 0xcd, 0x6c, 0x61,
		0x01, 0x81, 0xfc, 0x08,
		0x00, 0x11, 0x9b, 0x15,
	...
	return generic_ocp_write(tp, 0xd3cc, 0x3c, sizeof(u2phy_backup),
				 u2phy_backup, MCU_TYPE_USB);
}
...
static int r8159_init(struct r8152 *tp)
{
	...
	/* Disable USBIF u1u2 patch option */
	ret = ocp_byte_clr_bits(tp, MCU_TYPE_USB, 0xb9a6, BIT(0));
	if (ret < 0)
		goto out;
	...
	/* Enable u2 backup restore patch */
	if (tp->version == RTL_VER_17_QFN68) {
		ret = ocp_byte_set_bits(tp, MCU_TYPE_USB, 0xb99c, BIT(0));
		if (ret < 0)
			goto out;
	}

	/* Enable u3phy patch backup */
	ret = ocp_write_word(tp, MCU_TYPE_USB, 0xb9a2, 0x0448);
	if (ret < 0)
		goto out;
...
static int rtl8152_post_reset(struct usb_interface *intf)
{
	...
	if (determine_ethernet_addr(tp, &ss) >= 0) {
		...
		dev_set_mac_address(tp->netdev, (struct sockaddr *)&ss);
...
static bool delay_autosuspend(struct r8152 *tp)
{
	...
	if (!sw_linking && tp->rtl_ops.in_nway(tp))
		return true;
	else if (!skb_queue_empty_lockless(&tp->tx_queue))
		return true;
	else
		return false;
}
...
static int rtl8152_system_suspend(struct r8152 *tp, pm_message_t message)
{
	struct net_device *netdev = tp->netdev;
...
static int rtl8152_suspend(struct usb_interface *intf, pm_message_t message)
{
	...
	if (PMSG_IS_AUTO(message))
		...
		ret = rtl8152_system_suspend(tp, message);
			       ...
			       const void *d)
{
	...
	case ETHTOOL_TX_COPYBREAK:
		...
		if (val < ETH_ZLEN) {
			netif_err(tp, tx_err, netdev,
				  "Invalid tx copy break value\n");
			ret = -EINVAL;
			break;
		}

		if (tp->tx_copybreak != val) {
			ret = mutex_lock_interruptible(&tp->control);
			if (ret < 0)
				break;
			tp->tx_copybreak = val;
...
static int rtltool_ioctl(struct r8152 *tp, struct ifreq *ifr)
{
	...
	if (compat) {
		...
		myptr = (struct rtltool_cmd *)compat_ptr(ifr32->ifr_data);
	} else {
	...
	if (unlikely(compat && upper_32_bits((__u64)my_cmd.buf))) {
		netif_err(tp, drv, netdev,
			  "Bad address, the upper 4 bytes should be zero\n");
		return -EFAULT;
	}

	netif_dbg(tp, drv, netdev,
		  "%s: compat=%d cmd=%u offset=0x%x "
		  "byteen=0x%x data=0x%x buf=0x%llx\n",
		  __func__, compat, my_cmd.cmd, my_cmd.offset, my_cmd.byteen,
		  my_cmd.data, (__u64)my_cmd.buf);
	...
	case RTLTOOL_PLA_OCP_READ_DWORD:
		...
		netif_dbg(tp, drv, netdev, "%s: PLA_OCP_READ_DWORD data=0x%x\n",
			  __func__, my_cmd.data);
	...
	case RTLTOOL_PLA_OCP_WRITE_DWORD:
		...
		ret = pla_ocp_write(tp, (u16)my_cmd.offset, (u16)my_cmd.byteen,
	...
	case RTLTOOL_USB_OCP_READ_DWORD:
		...
		netif_dbg(tp, drv, netdev, "%s: USB_OCP_READ_DWORD data=0x%x\n",
			  __func__, my_cmd.data);
	...
	case RTLTOOL_USB_OCP_WRITE_DWORD:
		...
		ocp_data = __cpu_to_le32(my_cmd.data);
		ret = usb_ocp_write(tp, (u16)my_cmd.offset, (u16)my_cmd.byteen,
				    sizeof(ocp_data), &ocp_data);
		break;
	...
	case RTLTOOL_PLA_OCP_READ:
		...
		ret = pla_ocp_read(tp, (u16)my_cmd.offset, my_cmd.data, buffer);
		if (ret < 0) {
			kfree(buffer);
			break;
		}

		netif_dbg(tp, drv, netdev, "%s: PLA_OCP_READ data=%*ph\n",
			  __func__, (int)my_cmd.data, (u8 *)buffer);
	...
	case RTLTOOL_USB_OCP_READ:
		...
		ret = usb_ocp_read(tp, (u16)my_cmd.offset, my_cmd.data, buffer);
		if (ret < 0) {
			kfree(buffer);
			break;
		}

		netif_dbg(tp, drv, netdev, "%s: USB_OCP_READ data=%*ph\n",
			  __func__, (int)my_cmd.data, (u8 *)buffer);
	...
	case RTLTOOL_PHY_OCP_READ:
		...
		ocp_data = 0;
		ret = ocp_reg_read(tp, (u16)my_cmd.offset, (u16 *)&ocp_data);
		if (ret < 0)
			break;
		my_cmd.data = __le32_to_cpu(ocp_data);
		netif_dbg(tp, drv, netdev, "%s: PHY_OCP_READ data=0x%x\n",
			  __func__, my_cmd.data);
	...
	case RTLTOOL_USB_INFO:
		...
		ret = pla_ocp_read(tp, PLA_IDR, sizeof(uinfo->dev_addr),
				   uinfo->dev_addr);
		if (ret < 0)
			break;
...
static int rtl_hc_support_sg(struct usb_device *udev)
{
	struct device *hc_dev;
	struct pci_dev *pdev;
	int ret = 0;

	hc_dev = udev->bus ? udev->bus->controller : NULL;
	if (!hc_dev) {
		ret = -ENODEV;
		goto out;
	}

	if (hc_dev->bus != &pci_bus_type)
		goto out;

	pdev = to_pci_dev(hc_dev);
	dev_dbg(&udev->dev,
		"Host controller vendor ID = %#04x, device ID = %#04x\n",
		pdev->vendor, pdev->device);

	/*  ASM2142 fails to handle URBs with a large number of SG fragments,
	 *  causing the firmware freeze.
	 *  Fallback to linear buffering for stability.
	 */
	switch (pdev->vendor) {
			 ...
			 const char *buf, size_t count)
{
	...
	case 1:
		if (phy_rw) {
			ret = ocp_reg_read(tp, (u16)v1, (u16 *)&ocp_data);
			if (ret < 0)
				break;
			netif_info(tp, drv, netdev,
				   "PHY read word %#0hx = %#0hx\n", v1,
				   ocp_data);
		} else
			ret = -EINVAL;
		break;
	case 2:
		if (phy_rw) {
			netif_info(tp, drv, netdev,
				   "PHY write word %#0hx = %#0hx\n", v1, v2);
			ret = ocp_reg_write(tp, (u16)v1, (u16)v2);
			break;
		}
		...
		case 1:
			...
			if (ret < 0)
				break;
			netif_info(tp, drv, netdev, "%s read byte %#0hx = %#0hhx\n",
				   type ? "PLA" : "USB", v2,
				   ocp_data);
			break;
		case 2:
			...
			if (ret < 0)
				break;
			netif_info(tp, drv, netdev, "%s read word %#0hx = %#0hx\n",
				   type ? "PLA" : "USB", v2,
				   ocp_data);
			break;
		case 4:
			...
			if (ret < 0)
				break;
			netif_info(tp, drv, netdev, "%s read dword %#0hx = %#0x\n",
				   type ? "PLA" : "USB", v2,
				   ocp_data);
			break;
		default:
	...
	case 3:
		...
		switch (v1) {
		case 1:
			netif_info(tp, drv, netdev, "%s write byte %#0hx = %#0hhx\n",
				   type ? "PLA" : "USB", v2, v3);
			ret = ocp_write_byte(tp, type, v2, v3);
			break;
		case 2:
			netif_info(tp, drv, netdev, "%s write word %#0hx = %#0hx\n",
				   type ? "PLA" : "USB", v2, v3);
			ret = ocp_write_word(tp, type, v2, v3);
			break;
		case 4:
			netif_info(tp, drv, netdev, "%s write dword %#0hx = %#0x\n",
				   type ? "PLA" : "USB", v2, v3);
...
#else
static const struct bin_attribute *rtk_bin_attrs[] = {
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(6,16,0) */
	&bin_attr_pla,
	NULL
};
				...
				struct device_attribute *attr, char *buf)
{
	...
	ret = ocp_reg_read(tp, 0xbd84, &data);
	rtl_reset_ocp_base(tp);
...
static ssize_t tx_copybreak_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct net_device *netdev = to_net_dev(dev);
	struct r8152 *tp = netdev_priv(netdev);
...
static ssize_t tx_copybreak_store(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t count)
{
	struct net_device *netdev = to_net_dev(dev);
	...
	if (tx_copybreak < ETH_ZLEN)
		return -EINVAL;

	if (test_bit(RTL8152_UNPLUG, &tp->flags))
		return -ENODEV;

	if (tp->tx_copybreak != tx_copybreak) {
		int ret;
...
static DEVICE_ATTR_RW(tx_copybreak);
			 ...
			 const struct usb_device_id *id)
{
	...
	if (ret < 0) {
		dev_err(&intf->dev, "USB reset fails, errno = %d\n", ret);
		return ret;
	}
	...
	if (usb_device_no_sg_constraint(udev) && !rtl_hc_support_sg(udev))
		tp->sg_use = true;
	...
	tp->tx_copybreak = RTL8152_TXFG_HEADSZ;
...
static const struct usb_device_id rtl8152_table[] = {
	...
	/* ASUSTek */
	REALTEK_USB_DEVICE(VENDOR_ID_ASUSTEK, 0x1976),
	REALTEK_USB_DEVICE(VENDOR_ID_ASUSTEK, 0x1d91),
